...
export default function AdminTasks() {
    ...
    const handleSave = async () => {
        if (!formData.title.trim() || !formData.video_id || formData.questions.length === 0) {
            toast({
                title: 'Xatolik',
                description: 'Barcha maydonlarni to\'ldiring va kamida 1 ta savol qo\'shing',
                variant: 'destructive'
            });
            return;
        }
    ...
    const columns: Column<Task>[] = [
        ...
        //     render: (task) => (
        //         <div>
        //             <p className="text-sm font-medium">{getCategoryName(task.video) || 'Noma\'lum'}</p>
        ...
        },
        {
            ...
            render: (task) => (
                <span
                    className={`status-badge ${task.allow_resubmission ? 'bg-success/15 text-success' : 'bg-muted text-muted-foreground'}`}>
            ...
            render: (task) => (
                <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                    <Button
                        ...
                        className="h-8 w-8 text-primary hover:text-foreground"
                    >
