...
export default function AdminVideoEdit() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        toast({ title: 'Xatolik', description: getApiErrorMessage(error, 'Video maʼlumotlari yuklanmadi'), variant: 'destructive' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleVideoFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    video.onloadedmetadata = () => {
      ...
      setFormData(prev => ({ ...prev, duration: `${minutes}:${seconds.toString().padStart(2, '0')}`, videoUrl: '' }));
    };
    video.src = blobUrl;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Xatolik', description: getApiErrorMessage(error, 'Video tahrirlashda xatolik'), variant: 'destructive' });
    } finally {
      setSaving(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select value={formData.category} onValueChange={handleCategoryChange}>
                  ...
                  <SelectContent>
                    {categories.map(cat => <SelectItem key={cat.id} value={String(cat.id)}>{cat.icon} {cat.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
          ...
          <div className="rounded-xl border border-border bg-card p-6 space-y-4">
            ...
            {videoMode === 'file' ? (
              <div className="space-y-3">
                ...
                <Button type="button" variant="outline" className={`w-full ${videoError ? 'border-destructive' : ''}`} onClick={() => videoInputRef.current?.click()}>
                  ...
                  {videoFile ? videoFile.name : 'Yangi MP4 fayl tanlang'}
                </Button>
              ...
              <div className="space-y-3">
                <Input placeholder="https://www.youtube.com/watch?v=..." value={formData.videoUrl} onChange={(e) => setFormData(prev => ({ ...prev, videoUrl: e.target.value }))} />
                {formData.videoUrl && (
                  <div className="rounded-lg overflow-hidden border border-border bg-black aspect-video">
                    <iframe src={toEmbedUrl(formData.videoUrl)} title="Video preview" className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; fullscreen" allowFullScreen />
                  </div>
                )}
              </div>
            )}
            <div className="space-y-2">
              ...
              <Input id="duration" value={formData.duration} onChange={(e) => setFormData(prev => ({ ...prev, duration: e.target.value }))} placeholder="15:30" />
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-6 space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              <Input placeholder="Rasm URL" value={formData.thumbnail} onChange={(e) => { setFormData(prev => ({ ...prev, thumbnail: e.target.value })); setThumbnailPreview(e.target.value); }} />
              ...
              <Button type="button" variant="outline" onClick={() => thumbnailInputRef.current?.click()}>
                <ImageIcon className="mr-2 h-4 w-4" /> {thumbnailFile ? thumbnailFile.name : 'Rasm yuklash'}
              </Button>
            </div>
            {thumbnailPreview && (
              <div className="relative max-w-sm rounded-lg overflow-hidden">
                ...
                <Button type="button" variant="ghost" size="icon" className="absolute top-2 right-2 bg-background/80" onClick={() => { setThumbnailFile(null); setThumbnailPreview(''); setFormData(prev => ({ ...prev, thumbnail: '' })); }}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
            )}
          ...
          <div className="flex gap-3">
            <Button type="submit" disabled={saving || !!videoError} className="gradient-primary text-primary-foreground">
              <Save className="mr-2 h-4 w-4" />
              {saving ? 'Saqlanmoqda...' : 'Saqlash'}
            </Button>
            <Button type="button" variant="outline" onClick={() => navigate(`/admin/videos/${videoId}`)}>Bekor qilish</Button>
          </div>
        </form>
      </div>
    </DashboardLayout>
