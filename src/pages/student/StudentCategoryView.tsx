...
import {ArrowLeft, Search, LayoutGrid, Table2, Clock, CheckCircle2, Lock, Filter, Undo2} from 'lucide-react';
...
export default function StudentCategoryView() {
    ...
    // get data videos by categoryId

    const getCategory = async (categoryId: string) => {
        try {
            await api.get(`/categories/${categoryId}`).then((response) => {
    ...
    const getVideosByCategoryId = async (categoryId: string) => {
        try {
            api.get(`/videos/by_category/`, {
    ...
    const isVideoLocked = (video): boolean => {
